...
int main(void)
{
  ...
  {
	  timer_val = 0;
	  int output[OUTPUT_DEPTH][OUTPUT_HEIGHT][OUTPUT_WIDTH];
	if (flag==0) {
		for (int pwl = 0; pwl<20; pwl++) {
				...
				for (j = 0; j < OUTPUT_HEIGHT; j++) {
					for (k = 0; k < OUTPUT_WIDTH; k++) {
						...
						for (l = 0; l < K_C; l++) {
							for (m = 0; m < K_H; m++) {
								for (n = 0; n < K_W; n++) {
									sum += (long int)input[l][i+m][j+n] * kernel[l][m][n];
								}
							}
						}
						output[i][j][k] = sum;
		...
		//	HAL_Delay(50);
			...
			uart_buf_len = sprintf(uart_buf, "%u \t %ld us\r\n", timer_val, sum);
