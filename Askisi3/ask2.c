...
int open(unsigned char duty){
        OCR1AL=duty;
        unsigned char adcvalueh,adcvaluel;
        adcvaluel=ADCL;
        adcvalueh=ADCH;
       //PORTD = adcvalueh ;
        //PORTD =  ADCH;
...
int main(){
    ...
    while(1){
        ...
        inpout = PINB;
        _delay_ms(100);
        if(inpout==0b10011110){if(duty<255){duty+=20;}}// != exw arnitiki logiki
       
        else if(inpout==0b10011101){if(duty>13){duty-=20;} } // != exw arnitiki logiki
        calc();
        open(duty);
    }
}
